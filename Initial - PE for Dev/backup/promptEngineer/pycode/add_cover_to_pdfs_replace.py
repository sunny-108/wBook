...
replacing the original files with the new versions.
"""
...
logger = logging.getLogger('add_cover_page_replace')
...
def add_pages_to_pdfs():
    ...
    # Find all PDFs in the output directory (excluding temporary files and already processed files)
    pdf_files = [f for f in OUTPUT_DIR.glob("*.pdf") 
                if "temp" not in f.name and "_with_cover" not in f.name]
    
    for pdf_file in pdf_files:
        # Create a temporary output file
        temp_output_file = TEMP_DIR / f"temp_{pdf_file.name}"
        ...
        backup_file = TEMP_DIR / f"backup_{pdf_file.name}"
        shutil.copy2(pdf_file, backup_file)
        
        # Replace the original file with the new version
        shutil.copy2(temp_output_file, pdf_file)
        
        # Remove the temporary file
        temp_output_file.unlink()
        
        logger.info(f"Added cover and title page to {pdf_file} (original backed up at {backup_file})")

if __name__ == "__main__":
    try:
        add_pages_to_pdfs()
        print("Successfully added cover and title page to all PDFs in the output directory.")
        print("Original files were backed up in the output/temp directory.")
