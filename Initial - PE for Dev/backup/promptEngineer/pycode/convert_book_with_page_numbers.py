#!/usr/bin/env python
"""
Final script to convert Markdown chapter files to a single HTML file with PDF with working navigation
and accurate TOC page numbers.
"""
...
logger = logging.getLogger('md2pdf_with_pagenums')
...
def extract_headings_from_markdown(md_content: str) -> List[Tuple[int, str]]:
...
def convert_md_to_html_with_ids(md_content: str, chapter_num: Optional[int] = None) -> str:
...
def create_temporary_toc_html(toc_entries: List[Dict]) -> str:
    """Create HTML for table of contents with links to chapters - for initial document creation."""
    toc_html = """
    ...
    for entry in toc_entries:
        ...
        <li>
            <div class="toc-entry">
                ...
                <span class="page-number" id="page_chapter{chapter_num}"></span>
            </div>
            <ul>
        """
        ...
        for i, section in enumerate(entry['sections']):
            ...
            <li class="section-entry">
                <div class="toc-entry">
                    ...
                    <span class="dots"></span>
                    <span class="page-number" id="page_chapter{chapter_num}_section{section_num}"></span>
                </div>
            </li>
            """
...
def update_toc_with_page_numbers(html_content: str, page_numbers: Dict[str, int]) -> str:
    """Update the TOC HTML with actual page numbers."""
    updated_html = html_content
    
    for anchor_id, page_num in page_numbers.items():
        # Find placeholder in the TOC and replace with actual page number
        placeholder_pattern = f'<span class="page-number" id="page_{anchor_id}"></span>'
        replacement = f'<span class="page-number">{page_num}</span>'
        updated_html = updated_html.replace(placeholder_pattern, replacement)
...
def generate_complete_html(include_toc_placeholders=True) -> str:
...
"""
    ...
    # Add cover page
    complete_html = complete_html.format(css=CSS_STYLE) + create_cover_page_html()
    ...
    if about_book_path.exists():
        with open(about_book_path, 'r', encoding='utf-8') as f:
            about_book_content = f.read()
        about_book_html = convert_md_to_html_with_ids(about_book_content)
        complete_html += f'<div class="front-matter" id="about-book">{about_book_html}</div>'
    ...
    if about_author_path.exists():
        with open(about_author_path, 'r', encoding='utf-8') as f:
            about_author_content = f.read()
        about_author_html = convert_md_to_html_with_ids(about_author_content)
        complete_html += f'<div class="front-matter" id="about-author">{about_author_html}</div>'
    
    # Add table of contents with placeholders for page numbers
    if include_toc_placeholders:
        toc_html = create_temporary_toc_html(toc_entries)
    else:
        # This would be populated later with actual page numbers
        toc_html = "" 
    ...
    for chapter_file in chapter_files:
        ...
        if match:
            ...
            # Convert to HTML with IDs
            chapter_html = convert_md_to_html_with_ids(md_content, chapter_num)
            
            # Add chapter div with ID
            complete_html += f'<div class="chapter" id="chapter-content-{chapter_num}">{chapter_html}</div>'
...
def html_to_pdf(html_path: Path, pdf_path: Path) -> Tuple[Path, document.Document]:
    """Convert HTML file to PDF and return the Document object for page number calculation."""
    html = HTML(filename=str(html_path))
    doc = html.render(stylesheets=[CSS(string=CSS_STYLE)])
    doc.write_pdf(pdf_path)
    
    logger.info(f"Converted {html_path} to {pdf_path}")
    return pdf_path, doc

def extract_page_numbers(doc: document.Document) -> Dict[str, int]:
    """Extract page numbers for each heading in the document."""
    page_numbers = {}
    
    for page_idx, page in enumerate(doc.pages, 1):
        for anchor in page.anchors:
            page_numbers[anchor] = page_idx
    
    return page_numbers

def create_final_html_with_page_numbers(initial_html_path: Path, page_numbers: Dict[str, int]) -> Path:
    """Create final HTML with actual page numbers."""
    with open(initial_html_path, 'r', encoding='utf-8') as f:
        initial_html = f.read()
    
    # Update TOC with page numbers
    final_html = update_toc_with_page_numbers(initial_html, page_numbers)
    
    # Save updated HTML
    final_html_path = TEMP_HTML_DIR / "final_book.html"
    with open(final_html_path, 'w', encoding='utf-8') as f:
        f.write(final_html)
    
    logger.info(f"Created final HTML with page numbers: {final_html_path}")
...
def main():
    """Main function to convert markdown chapters to PDF with working navigation and page numbers."""
    try:
        # Ensure output directories exist
        ensure_directories_exist()
        
        # Generate initial HTML with placeholder page numbers
        initial_html = generate_complete_html(include_toc_placeholders=True)
        
        # Save initial HTML file
        initial_html_path = save_html_file(initial_html, "initial_book.html")
        
        # Generate PDF to calculate page numbers
        temp_pdf_path = TEMP_DIR / "temp_book.pdf"
        _, doc = html_to_pdf(initial_html_path, temp_pdf_path)
        
        # Extract page numbers for TOC entries
        page_numbers = {}
        for page_idx, page in enumerate(doc.pages, 1):
            for anchor in page.anchors:
                page_numbers[anchor] = page_idx
        ...
        # Create final HTML with actual page numbers
        final_html_path = create_final_html_with_page_numbers(initial_html_path, page_numbers)
        
        # Convert final HTML to PDF
        pdf_path = OUTPUT_DIR / "book_with_page_numbers.pdf"
        html_to_pdf(final_html_path, pdf_path)
        
        logger.info(f"Book successfully created with navigation and page numbers: {pdf_path}")
