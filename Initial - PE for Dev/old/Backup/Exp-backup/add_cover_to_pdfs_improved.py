...
This improved version only adds covers to PDFs that don't already have them.
"""
...
def add_pages_to_pdfs():
    """Add cover and title page to PDFs in the output directory that don't already have them."""
    # Create the cover and title page PDFs
    cover_pdf_path = create_cover_pdf()
    title_page_pdf_path = create_title_page_pdf()
    
    # Find all PDFs in the output directory (excluding those already with covers and temporary files)
    pdf_files = [
        f for f in OUTPUT_DIR.glob("*.pdf") 
        if "temp" not in f.name and "_with_cover" not in f.name
    ]
    
    # Skip if no files to process
    if not pdf_files:
        logger.info("No PDF files found that need covers. All files are already processed.")
        return
    ...
    for pdf_file in pdf_files:
        ...
        if output_file.exists():
            logger.info(f"Skipping {pdf_file}, {output_file} already exists.")
            continue
...
def clean_duplicate_covers():
    """Remove files that have multiple _with_cover suffixes."""
    duplicate_covers = [
        f for f in OUTPUT_DIR.glob("*_with_cover_with_cover*.pdf") 
    ]
    
    for file in duplicate_covers:
        try:
            os.remove(file)
            logger.info(f"Removed duplicate cover file: {file}")
        except Exception as e:
            logger.error(f"Failed to remove {file}: {e}")

if __name__ == "__main__":
    try:
        # First clean up any duplicate covers
        clean_duplicate_covers()
        
        # Then add covers to PDFs that don't have them
        add_pages_to_pdfs()
        
        print("Successfully added cover and title page to all PDF books.")
    except Exception as e:
